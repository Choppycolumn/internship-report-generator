...
- Git 默认忽略学校资料、用户导入的模板、每日照片、学生信息、打印机配置和所有生成文件。唯一明确公开的真实模板及配套文件见 [公开扫描模板](templates/README.md)，其余材料仍不上传。
...
仓库已包含经提供者明确同意公开的空白正文页：`lined_content_page_candidate`。原扫描 PDF、源图预览、校正图和毫米坐标配置均随仓库提供，克隆后无需重新导入。文件清单、尺寸来源及标定限制见 [模板说明](templates/README.md)。这不是《使用注意事项》要求页，也不包含个人实习内容。
...
该配置保留现有 `corrected` 状态和待复核标记；预览成功不等于完成实物套打校准。请按自己手中报告纸复核尺寸、正文区域和打印机偏移。
